...
function WarRoom() {
  ...
  // Track status for each stage: 'not_started' | 'running' | 'completed' | 'failed'
  const [stageStatuses, setStageStatuses] = createSignal<Record<PipelineStage, 'not_started' | 'running' | 'completed' | 'failed'>>(
    {} as Record<PipelineStage, 'not_started' | 'running' | 'completed' | 'failed'>
  );
      ...
      switch (msg.type) {
        ...
        case 'error':
          ...
          const errorLog: LogEntry = {
            ...
            stage: 'exception',
            timestamp: Date.now(),
            message: `Error: ${msg.payload?.message || 'Unknown error'}`,
          };
          setLogs((prev) => [...prev, errorLog].slice(-500));
          break;
        case 'theme:change':
          // Theme change handled by CSS variables
          break;
      }
    };
  ...
  const handlePipelineStatus = (payload: { stage: PipelineStage; message: string; data?: Record<string, unknown>; status?: 'running' | 'completed' | 'failed' }) => {
    ...
    // Update stage status
    setStageStatuses(prev => ({ ...prev, [newStage]: newStatus }));
  ...
  const handlePipelineComplete = (payload: { report: PipelineReport }) => {
    ...
    const completedStatuses: Record<PipelineStage, 'not_started' | 'running' | 'completed' | 'failed'> = {} as any;
    STAGE_ORDER.forEach(s => { completedStatuses[s] = 'completed'; });
    setStageStatuses(completedStatuses);
  ...
  const handlePipelineError = (payload: { error: string; stage: PipelineStage }) => {
    ...
    // Mark the failed stage as failed
    setStageStatuses(prev => ({ ...prev, [payload.stage]: 'failed' }));
  ...
  const renderHeaderBadge = () => {
    ...
    const badgeVariants: Record<string, 'default' | 'success' | 'danger'> = {
      not_started: 'default',
      running: 'default',
      completed: 'success',
      failed: 'danger',
    ...
    return (
      <Badge variant={badgeVariants[status] || 'default'} size="sm">
        {STAGE_LABELS[stage]} — {statusLabels[status]}
      </Badge>
    );
  };
        ...
        <span
          style={{
            ...
            background: status === 'completed'
              ? 'color-mix(in srgb, var(--vscode-testing-iconPassed) 15%, transparent)'
              : status === 'running'
              ...
              : 'color-mix(in srgb, var(--vscode-testing-iconFailed) 15%, transparent)',
  ...
  const renderStageStep = (stage: PipelineStage, index: number) => {
    ...
    const statusColors: Record<string, string> = {
      not_started: 'var(--vscode-descriptionForeground)',
      running: 'var(--vscode-button-background)',
  ...
  const renderLogEntry = (log: LogEntry) => {
    ...
    // Get status for this log's stage
    const stageStatus = stageStatuses()[log.stage] || 'not_started';
    const statusLabels: Record<string, string> = {
      not_started: 'Not Started',
      running: 'Running',
      completed: 'Complete',
    ...
    return (
      ...
      >
        ...
        <span style={{ 'font-size': '9px', 'font-weight': 600, 'text-transform': 'uppercase', 'letter-spacing': '0.3px', color: statusColors[stageStatus], flex: '0 0 80px', 'text-align': 'right' }}>

...
- Use assert statements to verify the exploit outcome (authoritative check)
- In addition to asserts, emit machine-readable evidence via console.log so the
  result can be parsed and money flow reconstructed. Use EXACTLY this format:
    console.log("Transfer(attacker, <protocol>, <amount>)");
    console.log("attacker <TOKEN> balance: <amount>");
  where <TOKEN> is the symbol of the stolen asset (e.g. ETH).
...
// PoCs can emit machine-readable evidence lines without the full forge-std lib.
library console {
...
export class PoCGenerator {
  ...
  async generate(request: PoCGenerationRequest): Promise<PoCResult> {
    ...
    // NOTE: Do NOT delete the temp workspace here. The generated PoC file
    // (test/PoC.t.sol) is consumed by the downstream compile + forge-test
    // stages in PipelineManager. The workspace is cleaned up by the caller
    // once the pipeline is finished (see PipelineManager.run).

...
export class OutputParser {
  ...
  parse(forgeOutput: ForgeOutput): {
    ...
    // Pattern A: "call <address>.<method>() returned <value>"
    ...
    // Pattern B: foundry's -vvv tree format, e.g.
    //   ├─ [1234] PoC::testExploit()
    //   ├─ [0] 0xabc...::withdraw(uint256) [staticcall]
    ...
    while ((match = treeTraceRegex.exec(raw)) !== null) {
      traces.push({
        from: 'unknown',
        to: match[1],
        method: match[2],
    ...
    // Derive genuine state changes from the PoC's emitted evidence. A PoC that
    // drains a protocol emits machine-readable money-flow + balance lines
    // (see POC_SYSTEM_PROMPT). Those ARE real evidence of state change and are
    // used to satisfy the HonestSignal "state change verified" condition.
    for (const entry of moneyFlow) {
      const amount = parseFloat(entry.amount) || 0;
      if (amount <= 0) continue;
      // Funds left `entry.to` (the protocol) and arrived at `entry.from`
      // (the attacker). Record both deltas.
      stateChanges.push({
        address: entry.to,
        change: -amount,
        oldValue: 'unknown',
    ...
    // Also derive from explicit "attacker <TOKEN> balance: <amount>" lines.
    const balRegex = /attacker\s+(\w+)\s+balance:\s*(\d+[.]?\d*)/gi;
    while ((match = balRegex.exec(raw)) !== null) {

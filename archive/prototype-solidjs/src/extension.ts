...
export function activate(context: vscode.ExtensionContext) {
  ...
  // -------------------------------------------------------------------------
  // 1. Initialize War Room Provider (fast, no I/O)
  // -------------------------------------------------------------------------
  warRoomProvider = new WarRoomProvider(context);

  // -------------------------------------------------------------------------
  // 2. License status bar - create item only, update in background
  // -------------------------------------------------------------------------
  licenseStatusBar = createLicenseStatusBarItem(context);
  context.subscriptions.push(licenseStatusBar);

  // -------------------------------------------------------------------------
  // 3. Register Sidebar Webview (minimal setup, lazy init)
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // 6. Defer ALL async initialization to background (after activate returns)
  // -------------------------------------------------------------------------
  setTimeout(() => {
...
function registerCommands(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sireen.newInvestigation', () => {
      // This command is invoked by the provider when it receives 'investigation:create'
      // The provider already handles creating the investigation and responding with 'investigation:created'
      // No action needed here - the provider handles it
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('sireen.runPipeline', async () => {
      const pm = getOrCreatePipelineManager();
    ...
    vscode.commands.registerCommand('sireen.enterLicense', async () => {
      ...
      if (key) {
        ...
        if (valid) {
          ...
          updateLicenseStatusBar(licenseStatusBar, context);
        } else {
...
async function initializeBackground(context: vscode.ExtensionContext): Promise<void> {
  try {
    // Initialize Pipeline Manager
    getOrCreatePipelineManager();
    sidebarProvider.setPipelineManager(pipelineManager!);

    // Initialize AI Client (loads API key from SecretStorage)
    sidebarProvider.reinitializeAIClient();
    ...
    console.warn('[Sireen] Background initialization failed:', err);
  }
}

function getOrCreatePipelineManager(): PipelineManager {
  if (!pipelineManager) {
...
async function runFirstTimeSetup(context: vscode.ExtensionContext) {
  ...
  // Check Docker and Foundry - fire and forget with short timeout, don't block activation
  setTimeout(() => {
    checkDockerAndFoundry().then((dockerCheck) => {
...
async function checkDockerAndFoundry(): Promise<{ docker: boolean; foundry: boolean }> {
  ...
  try {
    await execAsync('docker --version', { timeout: 500 });
    docker = true;
  } catch {}

  try {
    await execAsync('forge --version', { timeout: 500 });
    foundry = true;
  } catch {}
